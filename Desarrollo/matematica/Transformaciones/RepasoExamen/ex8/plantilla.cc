...
esat::Mat3 initMat(esat::Vec2 pos, int distance, float rot){
  esat::Mat3 temp;
  temp = esat::Mat3Identity();

  temp = esat::Mat3Multiply(esat::Mat3Translate(distance,distance), temp);
  temp = esat::Mat3Multiply(esat::Mat3Scale(30, 30), temp);
  temp = esat::Mat3Multiply(esat::Mat3Rotate(0.0005f * esat::Time() + rot), temp);
...
int esat::main(int argc, char** argv) {
  ...
  while (!esat::IsSpecialKeyDown(esat::kSpecialKey_Escape) && esat::WindowIsOpened()) {
    ...
    esat::Mat3 m = initMat(mousePosition, 0, 0);
    drawShape(m);
    float rotationSpeed = 1;
    m = initMat(mousePosition, 5, rotationSpeed);
    drawShape(m);
    rotationSpeed = 0.5f;
    m = initMat(mousePosition, 3, rotationSpeed);
    drawShape(m);
